...
public class Tree<T extends Comparable<T>> {
    private T val; //значение элемента
    private Tree left; // левый потомок
    private Tree right; // правый потомок
    private Tree parent; // родитель
    private List<T> listForPrint = new ArrayList<>(); // список всех элементов для вывода
    public T val() {
        return val;
    }
    public Tree left() {
    ...
    private Tree<T> _search(Tree<T> tree, T val){
        ...
        switch (val.compareTo(tree.val)){
            case 1: return _search(tree.right, val); // рекурсия правые элементы
            case -1: return _search(tree.left, val); // рекурсия левые элементы
            case 0: return tree;
            default: return null;
        }
    }
